...
\begin{ti}
\textbf{定理4.26(积分第二中值定理)}  设函数$f$在$[a,b]$上可积,$g$在$[a,b]$上单调,则存在$\xi\in[a,b]$,使得
...
如果欲证结论对$L^{-1}f$成立,则对$f$也成立.因此,不妨从一开始就假设$Jf(\mathbf{x}_0)=I_n$.在$\mathbf{x}_0=0$附近,
...
由\textbf{定理5.9}可知$\left\lVert g(x_1)-g(x_2)\right\rVert\leq\dfrac{1}{2}\left\lVert x_1-x_2\right\rVert$,$\forall x_1,x_2\in B_\delta(0)$.给定$y\in B_{\delta/2}(0)$,我们在$B_\delta(0)$中解方程$f(x)=y$,即$x=y-g(x)$.
...
\textbf{证明}:我们只证明这个定理的一个特殊情形,一般的情形可以借助下一章的单位分解定理转化为这种特殊情形.先做一些预备工作.\\
...
且
$$\iint_I\left(\frac{\partial Q}{\partial x}-\frac{\partial P}{\partial y}\right)\mathrm{d}x\mathrm{d}y=\int_0^{a}[Q(a,y)-Q(-a,y)]\,\mathrm{d}y-\int_{-a}^{a}[P(x,a)-P(x,0)]\,\mathrm{d}x=\int_{-a}^{a}P(x,0)\,\mathrm{d}x,$$
这说明$Green$公式在$I$上成立.\\
...
为此,设$\varphi:\widetilde\Omega\to\Omega$为$C^2$的保定向坐标变换,记为$\varphi(u,v,w)=(x(u,v,w),y(u,v,w),z(u,v,w)),\,(u,v,w)\in\widetilde\Omega$.利用复合求导的链式法则以及第二型曲面积分的定义不难得到
$$\iint_{\partial\widetilde\Omega}\overline P\,\mathrm{d}v\wedge\mathrm{d}w+\overline Q\,\mathrm{d}w\wedge\mathrm{d}u+\overline R\,\mathrm{d}u\wedge\mathrm{d}v=\iint_{\partial\Omega}P\,\mathrm{d}y\wedge\mathrm{d}z+Q\,\mathrm{d}z\wedge\mathrm{d}x+R\,\mathrm{d}x\wedge\mathrm{d}y,$$
...
在上式中将$(y,z)$换成$(z,x)$或$(x,y)$也成立.利用这些等式以及$\varphi$保持定向可知,$x_uy_vz_w$的符号不变,再由重积分的变量替换公式就得到
...
\begin{ti}
\textbf{定理6.6(比较判别法)}  设$\displaystyle\sum a_n$和$\displaystyle\sum b_n$为正项级数,如果存在$M>0,N\geq 1$,使得当$n>N$时$a_n\leq Mb_n$,则$\displaystyle\sum b_n$收敛时$\displaystyle\sum a_n$也收敛;$\displaystyle\sum a_n$发散时$\displaystyle\sum b_n$也发散.
\end{ti}
...
\begin{ti}
\textbf{定理6.12(无穷乘积的收敛判别法)}  设$p_n$均大于零,记$p_n=1+a_n$,则\\
...
\begin{ti}
\textbf{定理6.13(级数的乘积)}  设级数$\displaystyle\sum_{n=0}^{\infty}a_n$与$\displaystyle\sum_{n=0}^{\infty}b_n$绝对收敛,定义它们的乘积为级数$\displaystyle\sum_{n=0}^{\infty}c_n$,其中$c_n=\displaystyle\sum_{i+j=n}a_i b_j$,$n\geq 0$.这种乘积称为\textbf{Cauchy乘积}.则乘积级数$\displaystyle\sum_{n=0}^{\infty}c_n$也绝对收敛,且
...
以及正项级数$\displaystyle\sum\frac{a_n+|a_n|}{2}$与$\displaystyle\sum\frac{|a_n|-a_n}{2}$的收敛性可知,重排后的级数也绝对收敛,且其和不变.

\begin{ti}
\textbf{定理6.15(Riemann重排定理)}  如果级数$\displaystyle\sum a_n$条件收敛,则可以将它重排为一个收敛级数,使得重排后的级数和为任意指定的实数.
\end{ti}
...
\subsection{函数项级数}
\begin{ti}
\textbf{定义6.16(一致收敛)}  设$I$为区间,$\{f_n\}$为$I$中定义的一列函数.如果对任意的$\varepsilon>0$,均存在与$x$无关的正整数$N=N(\varepsilon)$,使得当$n>N,x\in I$时,$\left\lvert f_n(x)-f(x)\right\rvert<\varepsilon$总成立,则称函数列$\{f_n\}$在$I$中\textbf{一致收敛}于$f$,记为$f_n\Rightarrow f$.
对于函数项级数$\displaystyle\sum f_n(x)$,若其部分和函数列一致收敛,则称该级数在$I$上\textbf{一致收敛}.
\end{ti}

\begin{ti}
\textbf{定理6.17(一致收敛的Cauchy准则)}  定义在$I$中的函数列$\{f_n\}$一致收敛当且仅当对任意的$\varepsilon>0$,存在$N=N(\varepsilon)$,使得当$m,n>N,x\in I$时,$\left\lvert f_m(x)-f_n(x)\right\rvert<\varepsilon$.
\end{ti}
...
\begin{ti}
\textbf{定理6.18(Weierstrass判别法)}  如果$|f_n(x)|\leq M_n$,而正项级数$\displaystyle\sum M_n$收敛,则$\displaystyle\sum f_n(x)$在$I$中一致收敛.
\end{ti}
...
\begin{ti}
\textbf{定理6.20(逐项积分)}  设$f_n\in R[a,b]$,如果$\{f_n\}$一致收敛于$f$,则$f\in R[a,b]$,且
$$\lim_{n\to\infty}\int_a^b f_n(x)\,\mathrm{d}x=\int_a^b \lim_{n\to\infty}f_n(x)\,\mathrm{d}x=\int_a^b f(x)\,\mathrm{d}x$$.
\end{ti}

\textbf{证明}:先来说明$f$可积.任给$\varepsilon>0$,存在$N=N(\varepsilon)$,当$n\geq N,x\in[a,b]$时,$|f_n(x)-f(x)|\leq\dfrac{\varepsilon}{4(b-a)}$.因为$f_N$是可积函数,故存在$[a,b]$的分割$\pi$,使得$\displaystyle\sum_i\omega_i(f_N)\Delta x_i<\dfrac{\varepsilon}{2}$.对于每一个小区间$[x_{i-1},x_i]$,由上式可得
...
$$\left\lvert\int_a^b f_n(x)\,\mathrm{d}x-\int_a^b f(x)\,\mathrm{d}x\right\rvert\leq\int_a^b|f_n(x)-f(x)|\,\mathrm{d}x\leq\frac{\varepsilon}{4},$$
这说明$\displaystyle\lim_{n\to\infty}\int_a^b f_n(x)\,\mathrm{d}x=\int_a^b f(x)\,\mathrm{d}x$.

\begin{ti}
\textbf{定理6.21(逐项求导)}  设$\{f_n\}$为$[a,b]$中的一列可导函数,$c\in[a,b]$.如果$\{f_n(c)\}$收敛,$\{f'_n\}$一致收敛于$g$,
...
根据前节定理6.19,$\widehat{g}$为连续函数.特别地,$f$在$x_0$处可导且导数为$g(x_0)$.
...
\begin{ti}
\textbf{定理6.23(幂级数的性质)}  设幂级数$\displaystyle\sum_{n=0}^{\infty}a_n x^n$的收敛半径为$R$,则其和函数$S(x)=\displaystyle\sum_{n=0}^{\infty}a_n x^n$在$(-R,R)$中任意次可导,且
$$S^{(k)}(x)=\sum_{n=k}^{\infty}n(n-1)\cdots(n-k+1)a_n x^{n-k};$$
并且当$x\in(-R,R)$时,有
$$\int_0^x S(t)\,\mathrm{d}t=\sum_{n=0}^{\infty}\frac{a_n}{n+1}x^{n+1}.$$
\end{ti}

\textbf{证明}:以$k=1$为例.首先,幂级数$\displaystyle\sum_{n=0}^{\infty}(a_n x^n)'=\sum_{n=1}^{\infty}na_n x^{n-1}$的收敛半径仍为$R$,故它在闭区间$I\subset(-R,R)$中一致收敛.由定理6.21可知,$\displaystyle\sum_{n=0}^{\infty}a_n x^n$在$I$中可导,且
...
逐项积分:不妨设$x>0$,根据前面的讨论,$\displaystyle\sum_{n=0}^{\infty}a_n t^n$在$[0,x]$中一致收敛,因此可以逐项积分,即得$\displaystyle\int_0^x S(t)\,\mathrm{d}t=\sum_{n=0}^{\infty}\frac{a_n}{n+1}x^{n+1}$.
...
$$\frac{1}{\pi}\int_{-\pi}^{\pi}S_n(x)g(x)\,\mathrm{d}x=\frac{1}{\pi}\left[\frac{a_0}{2}\int_{-\pi}^{\pi}g(x)\,\mathrm{d}x+\sum_{k=1}^{n}\left(a_k\int_{-\pi}^{\pi}g(x)\cos kx\,\mathrm{d}x+b_k\int_{-\pi}^{\pi}g(x)\sin kx\,\mathrm{d}x\right)\right]=\frac{1}{2}a_0\alpha_0+\sum_{k=1}^{n}(a_k\alpha_k+b_k\beta_k).$$
特别地,在上式中分别取$g(x)=S_n(x),f(x)$,注意到$S_n(x)$的Fourier展开是它自身,可以得到
$$\frac{1}{\pi}\int_{-\pi}^{\pi}S_n(x)^2\,\mathrm{d}x=\frac{1}{2}a_0^2+\sum_{k=1}^{n}(a_k^2+b_k^2),$$
$$\frac{1}{\pi}\int_{-\pi}^{\pi}S_n(x)f(x)\,\mathrm{d}x=\frac{1}{2}a_0^2+\sum_{k=1}^{n}(a_k^2+b_k^2),$$
由此立得$Bessel$不等式.
...
\begin{ti}
\textbf{定理7.5(Dirichlet收敛定理)}  设$f$是周期为$2\pi$的分段可导函数,$x\in[-\pi,\pi]$,则$f$的Fourier展开在$x$处收敛到
$$\frac{f(x+0)+f(x-0)}{2}$$
...
其中
$$E_n(x)=\frac{1}{\pi}\int_0^{\pi}\frac{f(x+u)-f(x+0)}{2\sin\frac{u}{2}}\sin\left(n+\frac{1}{2}\right)u\,\mathrm{d}u+\frac{1}{\pi}\int_0^{\pi}\frac{f(x-u)-f(x-0)}{2\sin\frac{u}{2}}\sin\left(n+\frac{1}{2}\right)u\,\mathrm{d}u.$$
如果$f$分段可导,则$\dfrac{f(x+u)-f(x+0)}{2\sin\frac{u}{2}}$和$\dfrac{f(x-u)-f(x-0)}{2\sin\frac{u}{2}}$关于$u$均为分段连续函数,从而应用$Riemann-Lebesgue$引理即得欲证结论.特别地,在$f$的连续点处,$f(x+0)=f(x-0)=f(x)$,故收敛到$f(x)$.

\begin{ti}
\textbf{定理7.6(一致收敛)}  设$f$是以$2\pi$为周期的连续函数,如果$f$可以写成两个单调函数之差,则其$Fourier$级数一致收敛于$f$自身.特别地,$Lipschitz$周期函数的$Fourier$级数一致收敛于自身.
\end{ti}
...
$$\lim_{n\to\infty}\frac{1}{\pi}\int_{-\pi}^{\pi}f_n^2(x)\,\mathrm{d}x=\frac{1}{\pi}\int_{-\pi}^{\pi}f^2(x)\,\mathrm{d}x.$$
注意到$f_n$的$Fourier$展开一致收敛于自身:
$$f_n(x)=\frac{a_{n,0}}{2}+\sum_{k=1}^{\infty}(a_{n,k}\cos kx+b_{n,k}\sin kx).$$
上式两边乘$f_n(x)$以后仍然一致收敛,逐项积分可得
$$\frac{1}{\pi}\int_{-\pi}^{\pi}f_n^2(x)\,\mathrm{d}x=\frac{1}{2}a_{n,0}^2+\sum_{k=1}^{\infty}(a_{n,k}^2+b_{n,k}^2).$$
同理,乘$f(x)$再积分可得
$$\frac{1}{\pi}\int_{-\pi}^{\pi}f_n(x)f(x)\,\mathrm{d}x=\frac{1}{2}a_{n,0}a_0+\sum_{k=1}^{\infty}(a_{n,k}a_k+b_{n,k}b_k).$$
利用平均值不等式$ab\leq\dfrac{a^2+b^2}{2}$可得
$$\frac{1}{\pi}\int_{-\pi}^{\pi}f_n(x)f(x)\,\mathrm{d}x\leq\frac{1}{2}\left[\frac{1}{2}a_{n,0}^2+\sum_{k=1}^{\infty}(a_{n,k}^2+b_{n,k}^2)\right]+\frac{1}{2}\left[\frac{1}{2}a_0^2+\sum_{k=1}^{\infty}(a_k^2+b_k^2)\right].$$
令$n\to\infty$,可得
$$\frac{1}{\pi}\int_{-\pi}^{\pi}f^2(x)\,\mathrm{d}x\leq\frac{1}{2}\left[\frac{1}{\pi}\int_{-\pi}^{\pi}f^2(x)\,\mathrm{d}x+\frac{1}{2}a_0^2+\sum_{k=1}^{\infty}(a_k^2+b_k^2)\right],$$
