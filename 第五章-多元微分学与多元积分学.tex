...
唯一零点$\displaystyle f(x)$位于$\displaystyle(f(x^*)-\rho,f(x^*)+\rho)$内,故$\displaystyle|f(x)-f(x^*)|<\rho<\varepsilon$, \\
...
$=\displaystyle\int_0^{2\pi}\dfrac{\partial u}{\partial x}(a_1+r\cos\theta,a_2+r\sin\theta)\,r\cos\theta\,d\theta$ \\
$\quad+\displaystyle\int_0^{2\pi}\dfrac{\partial u}{\partial y}(a_1+r\cos\theta,a_2+r\sin\theta)\,r\sin\theta\,d\theta$ \\
...
\textbf{微分学视角}:假设$u$不恒为$0$.不妨设$u$的最大值大于$0$,否则对$-u$作同样论证.由边界条件,存在$x^0\in int\Omega$,$\;\text{s.t.}\,u(x^0)>0$, \\
