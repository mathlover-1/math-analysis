...
令$A=\{t\in[a,b]\mid f\text{在}[a,t]\text{上恒等于}f(a)\}$,则$a\in A$.取$s=\sup A$. \\

任意$t\in[a,s)$,由上确界的定义可取$r\in A$使$t<r\leq s$,故$f(t)=f(a)$. \\

若$s=a$则$s\in A$显然;若$s>a$,由$f$在$s$处局部常值,可取$t<s$且充分接近$s$,从而$f(s)=f(t)=f(a)$.因此$s\in A$. \\

若$s<b$,则由$f$在$s$处局部常值,可取$y\in(s,b]$使$f$在$[s,y]$上恒等于$f(s)$,这与$s=\sup A$矛盾.故$s=b$,闭区间上的结论得证. \\

对于开区间,任取其中两点$x<y$,限制在内部闭区间$[x,y]$上应用上述结论,即得$f(x)=f(y)$.其余类型区间同理. \\
